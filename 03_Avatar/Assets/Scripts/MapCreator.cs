...
public class MapCreator : MonoBehaviour
{
    ...
    public void setAvatarPositionUTM(double x, double y, String z, int fix, int avatarID)
    {
        ...
        else
        {
            ...
            {          
                avatars.TryGetValue(avatarID, out a);
            }
            else
            {
                a = Instantiate(avatarPrefap);
                avatars.Add(avatarID, a);
            }
            a.transform.localPosition = new Vector3(x_ * 10, mapObjY + 1f, y_ * 10);  //10 times bc size of plane
        }
    }
}
